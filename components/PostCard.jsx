...
export default function PostCard({ post }) {
  ...
  const handleKeyDown = e => {
    // Don't navigate if focus is on a link (tag)
    if (e.target.tagName === "A" || e.target.closest("a")) {
      return;
    }
  ...
  return (
    <article
      ...
      tabIndex={0}
    >
      <div className="bg-gray-800 rounded-lg shadow-md overflow-hidden hover:shadow-2xl hover:shadow-blue-900/30 group-focus:shadow-2xl group-focus:shadow-blue-900/30 transition-all duration-300 border border-gray-700 hover:border-blue-500 group-focus:border-blue-500 hover:-translate-y-1 group-focus:-translate-y-1">
        {post.image && (
